...
(see xref:../guide-configuration.adoc[configuration guide]). 
...
xref:../coding-conventions.adoc#packages[packaging conventions].
...
* `${type}` to the type of the service. E.g. `rest` in case of a xref:../guide-rest.adoc[REST] service and `ws` for a xref:../guide-soap.adoc[SOAP] service.
...
=== Headers
A very common demand is to tweak (HTTP) headers in the request to invoke the service. May it be for security (authentication data) or for other cross-cutting concerns (such as the xref:../guide-logging.adoc#correlation-id[Correlation ID]). This is done internally by implementations of the interface  `ServiceHeaderCustomizer`.
...
* `ServiceHeaderCustomizerCorrelationId` passed the xref:../guide-logging.adoc#correlation-id[Correlation ID] to the service request.
...
* *Service error* +
In case the service failed on the server-side, the xref:../guide-rest.adoc#error-results[error result] will be parsed and thrown as a `ServiceInvocationFailedException` with the received message and code.
